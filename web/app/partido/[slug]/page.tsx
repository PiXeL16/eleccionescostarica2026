...
export default async function PartyDetailPage({ params }: PageProps) {
  ...
  const partyOrganizationSchema = generatePartyOrganizationSchema(party);
  const breadcrumbSchema = generateBreadcrumbSchema(
    breadcrumbItems.map((item) => ({ name: item.name, url: `${SITE_URL}${item.url}` }))
  );
  const candidateSchema = candidate ? generateCandidatePersonSchema(candidate, party) : null;
  ...
  return (
    <>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(partyOrganizationSchema) }}
      />
      <script
        type="application/ld+json"
